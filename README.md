...
A* cost는 단순 최단거리만 보지 않는다. 경로 계획에서는 hard penalty와 soft penalty를 나누어 사용했다.

- Hard penalty: 벽, 맵 경계, 점유된 주차 슬롯, 차량 직사각형 기준으로 실제 충돌이 발생하는 위치는 경로 후보에서 제외한다. 이 영역은 비용을 높이는 수준이 아니라 통과할 수 없는 장애물로 처리한다.
